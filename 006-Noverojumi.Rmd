...
rezgis=rezgis %>% 
  mutate(tikls100=id,
         tikls1000=ID1km) %>% 
  dplyr::select(tikls100,tikls1000)


# LVM 2023 a
lvm=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_ACCGEN_LVM_2023x.xlsx")
table(lvm$ligzdas_statuss,useNA = "always")
lvm2=lvm %>% 
  mutate(datums_no=as.Date(paste0(gads,"-06-15")),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=ifelse(ligzdas_statuss=="Ligzdojošs pāris","AL",
                       ifelse(ligzdas_statuss %in% c("Pāris ar neskaidru statusu","Teritoriāls pāris"),"P",NA)),
         BreedCat=ifelse(pazime=="AL","Pierādīta ligzdošana",
                         ifelse(pazime=="P","Ticama ligzdošana",NA)),
         BreedCode=ifelse(BreedCat=="Pierādīta ligzdošana",1,
                         ifelse(BreedCat=="Ticama ligzdošana",2,NA)),
         lksX=x_koordinate,
         lksY=y_koordinate,
         avots="eksperti_LVM",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
lvm2_sf=st_as_sf(lvm2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
lvm2_sf$rinda=rownames(lvm2_sf)
lvm2_kvad=st_join(lvm2_sf,rezgis)
lvm_fin=data.frame(lvm2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(lvm_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_LVM.parquet")
rm(lvm)
rm(lvm2)
rm(lvm2_kvad)
rm(lvm2_sf)
...
# citas LVM-projekta ligzdas
lvm_citasL=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_LVMproj_citi_ligzdas2024.xlsx")
table(lvm_citasL$pazime,useNA = "always")
lvm_citasL2=lvm_citasL %>% 
  mutate(gads=lubridate::year(datums),
         datums_no=as.Date(datums),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazime,
         BreedCat=ifelse(pazime=="AL","Pierādīta ligzdošana",NA),
         BreedCode=ifelse(BreedCat=="Pierādīta ligzdošana",1,NA),
         lksX=ligzda_X,
         lksY=ligzda_Y,
         avots="eksperti_LVM",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
lvm_citasL2_sf=st_as_sf(lvm_citasL2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
lvm_citasL2_sf$rinda=rownames(lvm_citasL2_sf)
lvm_citasL2_kvad=st_join(lvm_citasL2_sf,rezgis)
lvm_citasL2_fin=data.frame(lvm_citasL2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(lvm_citasL2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_LVM_LUligzdas.parquet")
rm(lvm_citasL)
rm(lvm_citasL2)
rm(lvm_citasL2_kvad)
rm(lvm_citasL2_sf)
...
# citi LVM-projekta novērojumi
lvm_citiN=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_LVMproj_citi_nov2024.xlsx")

table(lvm_citiN$pazime,useNA = "always")

lvm_citiN2=lvm_citiN %>% 
  mutate(gads=lubridate::year(DateTime),
         datums_no=as.Date(DateTime),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazime,
         BreedCat=ifelse(pazime=="AL"|pazime=="RM","Pierādīta ligzdošana",
                         ifelse(pazime=="U","Ticama ligzdošana",
                                ifelse(pazime=="B"|pazime=="D","Iespējama ligzdošana",NA))),
         BreedCode=ifelse(BreedCat=="Pierādīta ligzdošana",1,
                          ifelse(BreedCat=="Ticama ligzdošana",2,
                                 ifelse(BreedCat=="Iespējama ligzdošana",3,NA))),
         lksX=putns_X,
         lksY=putns_Y,
         avots="eksperti_LVM",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
lvm_citiN2_sf=st_as_sf(lvm_citiN2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
lvm_citiN2_sf$rinda=rownames(lvm_citiN2_sf)
lvm_citiN2_kvad=st_join(lvm_citiN2_sf,rezgis)
lvm_citiN2_fin=data.frame(lvm_citiN2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(lvm_citiN2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_LVM_LUnov.parquet")
rm(lvm_citiN)
rm(lvm_citiN2)
rm(lvm_citiN2_kvad)
rm(lvm_citiN2_sf)
...
# dd
dd=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_dd_noverojumi.xlsx")
table(dd$pazimei,useNA = "always")
dd2=dd %>% 
  mutate(gads=lubridate::year(datumam),
         datums_no=as.Date(datumam),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazimei,
         BreedCat=ifelse(pazime %in% c("LM","LO","JB","AL","LL","RM","AU"),"Pierādīta ligzdošana",
                         ifelse(pazime %in% c("G","L","U","V","R","P","T"),"Ticama ligzdošana",
                                ifelse(pazime=="B"|pazime=="D","Iespējama ligzdošana",NA))),
         BreedCode=ifelse(BreedCat=="Pierādīta ligzdošana",1,
                          ifelse(BreedCat=="Ticama ligzdošana",2,
                                 ifelse(BreedCat=="Iespējama ligzdošana",3,NA))),
  ...
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
dd2_sf=st_as_sf(dd2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
dd2_sf$rinda=rownames(dd2_sf)
dd2_kvad=st_join(dd2_sf,rezgis)
dd2_fin=data.frame(dd2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(dd2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_dd.parquet")
rm(dd)
rm(dd2)
rm(dd2_kvad)
rm(dd2_sf)
...
# kalvans
kalvans=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_Kalvans.xlsx")
table(kalvans$pazime,useNA = "always")
kalvans2=kalvans %>% 
  mutate(datums_no=as.Date(datumamXXX),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazime,
         BreedCat=ifelse(pazime %in% c("LM","LO","JB","AL","LL","RM","AU"),"Pierādīta ligzdošana",
         ...
         lksX=x,
         lksY=y,
         avots="eksperti_Kalvans",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
kalvans2_sf=st_as_sf(kalvans2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
kalvans2_sf$rinda=rownames(kalvans2_sf)
kalvans2_kvad=st_join(kalvans2_sf,rezgis)
kalvans2_fin=data.frame(kalvans2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(kalvans2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_kalvans.parquet")
rm(kalvans)
rm(kalvans2)
rm(kalvans2_sf)
rm(kalvans2_kvad)
...
# kuze
kuze=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_Kuze.xlsx")
table(kuze$pazimai,useNA = "always")
kuze2=kuze %>% 
  mutate(datums_no=as.Date(datumamXXX),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
  ...
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
kuze2_sf=st_as_sf(kuze2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
kuze2_sf$rinda=rownames(kuze2_sf)
kuze2_kvad=st_join(kuze2_sf,rezgis)
kuze2_fin=data.frame(kuze2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(kuze2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_kuze.parquet")
rm(kuze)
rm(kuze2)
rm(kuze2_sf)
rm(kuze2_kvad)
...
# DabasFonds
DabasFonds=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_LDF_ligzdas.xlsx")
table(DabasFonds$pazime,useNA = "always")
DabasFonds2=DabasFonds %>% 
  mutate(datums_no=as.Date(datumamXXX),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         ...
         lksX=x,
         lksY=y,
         avots="eksperti_DabasFonds",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
DabasFonds2_sf=st_as_sf(DabasFonds2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
DabasFonds2_sf$rinda=rownames(DabasFonds2_sf)
DabasFonds2_kvad=st_join(DabasFonds2_sf,rezgis)
DabasFonds2_fin=data.frame(DabasFonds2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(DabasFonds2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_DabasFonds.parquet")
rm(DabasFonds)
rm(DabasFonds2)
rm(DabasFonds2_sf)
rm(DabasFonds2_kvad)
...
# LedinsJakovlevs
LedinsJakovlevs=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_LedinsJakovlevs.xlsx")
table(LedinsJakovlevs$pazimei,useNA = "always")
LedinsJakovlevs2=LedinsJakovlevs %>% 
  mutate(datums_no=as.Date(datumamXXX),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         ...
         lksX=x,
         lksY=as.numeric(y),
         avots="eksperti_LedinsJakovlevs",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
LedinsJakovlevs2_sf=st_as_sf(LedinsJakovlevs2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
LedinsJakovlevs2_sf$rinda=rownames(LedinsJakovlevs2_sf)
LedinsJakovlevs2_kvad=st_join(LedinsJakovlevs2_sf,rezgis)
LedinsJakovlevs2_fin=data.frame(LedinsJakovlevs2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(LedinsJakovlevs2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_LedinsJakovlevs.parquet")
rm(LedinsJakovlevs)
rm(LedinsJakovlevs2)
rm(LedinsJakovlevs2_sf)
rm(LedinsJakovlevs2_kvad)
rm(LedinsJakovlevs2_fin)




# PucuPlans
PucuPlans=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_pucu_plans.xlsx")
table(PucuPlans$Vecums,useNA = "always")
table(PucuPlans$Dzimums,useNA = "always")
PucuPlans2=PucuPlans %>% 
  mutate(datums_no=as.Date(Datums,"%Y.%m.%d."),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=ifelse(Vecums=="juv","RM",
                       ifelse(Dzimums=="M"|Dzimums=="P","P","D")),
         BreedCat=ifelse(pazime %in% c("LM","LO","JB","AL","LL","RM","AU"),"Pierādīta ligzdošana",
                         ifelse(pazime %in% c("G","L","U","V","R","P","T"),"Ticama ligzdošana",
                                ifelse(pazime=="B"|pazime=="D","Iespējama ligzdošana",NA))),
         BreedCode=ifelse(BreedCat=="Pierādīta ligzdošana",1,
         ...
         lksX=puce_X,
         lksY=puce_Y,
         avots="eksperti_PucuPlans",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
PucuPlans2_sf=st_as_sf(PucuPlans2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
PucuPlans2_sf$rinda=rownames(PucuPlans2_sf)
PucuPlans2_kvad=st_join(PucuPlans2_sf,rezgis)
PucuPlans2_fin=data.frame(PucuPlans2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(PucuPlans2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_PucuPlans.parquet")
rm(PucuPlans)
rm(PucuPlans2)
rm(PucuPlans2_sf)
rm(PucuPlans2_kvad)
...
# Racinskis
Racinskis=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_Racinskis.xls")
table(Racinskis$pazime,useNA = "always")
Racinskis2=Racinskis %>% 
  mutate(datums_no=as.Date(datumamXXX),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         ...
         lksX=x,
         lksY=y,
         avots="eksperti_Racinskis",
         DatuKopa="eksperti") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
Racinskis2_sf=st_as_sf(Racinskis2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
Racinskis2_sf$rinda=rownames(Racinskis2_sf)
Racinskis2_kvad=st_join(Racinskis2_sf,rezgis)
Racinskis2_fin=data.frame(Racinskis2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(Racinskis2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_Racinskis.parquet")
rm(Racinskis)
rm(Racinskis2)
rm(Racinskis2_sf)
rm(Racinskis2_kvad)
...
# Plesputni
Plesputni=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_plesu_dati.xlsx")
table(Plesputni$pazimei,useNA = "always")
Plesputni2=Plesputni %>% 
  mutate(datums_no=as.Date(datumam),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazimei,
         ...
         lksX=X,
         lksY=Y,
         avots="monitoringi_plesputni",
         DatuKopa="monitoringi") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
Plesputni2_sf=st_as_sf(Plesputni2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
Plesputni2_sf$rinda=rownames(Plesputni2_sf)
Plesputni2_kvad=st_join(Plesputni2_sf,rezgis)
Plesputni2_fin=data.frame(Plesputni2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(Plesputni2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_Plesputni.parquet")
rm(Plesputni)
rm(Plesputni2)
rm(Plesputni2_sf)
rm(Plesputni2_kvad)
...
# LPU
LPU=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_lpu2023.xlsx")
table(LPU$pazimei,useNA = "always")
LPU2=LPU %>% 
  mutate(datums_no=as.Date(datumam),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         ...
         lksX=X,
         lksY=Y,
         avots="monitoringi_LPU",
         DatuKopa="monitoringi") %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
LPU2_sf=st_as_sf(LPU2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
LPU2_sf$rinda=rownames(LPU2_sf)
LPU2_kvad=st_join(LPU2_sf,rezgis)
LPU2_fin=data.frame(LPU2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(LPU2_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_LPU.parquet")
rm(LPU)
rm(LPU2)
rm(LPU2_sf)
rm(LPU2_kvad)
...
# dabasdati
dabasdati=read_parquet("./IevadesDati/Noverojumi/nosaukumi/vardnica_putni_DABASDATI_vardnica.parquet")
table(dabasdati$Ticamiba,useNA="always")
ddati=dabasdati %>% 
  filter(Ticamiba!="Apzinātas muļķības") %>% 
  filter(Ticamiba!="Biežas kļūdas elementārās lietās") %>% 
  filter(!is.na(Ticamiba))
table(dabasdati$statuss,useNA="always")
ddati=ddati %>% 
  mutate(pazimei=case_when(statuss=="Aizvilinoša vai uzbrūkoša uzvedība (AU)"~"AU",
                           statuss=="Apdzīvota ligzda vai dobums (AL)"~"AL",
                           statuss=="Apmeklē iespējamu ligzdas vietu (V)"~"V",
                           statuss=="Baro mazuļus (JB)"~"JB",
                           statuss=="Dzied ligzdošanai piemērotā biotopā (D)"~"D",
                           statuss=="Gatavo ligzdu (nes ligzdas materiālu) (G)"~"G",
                           statuss=="Lietota ligzda (LL)"~"LL",
                           statuss=="Ligzda ar mazuļiem (LM)"~"LM",
                           statuss=="Ligzda ar mazuļiem (piezīmēs norādiet mazuļu skaitu)"~"LM",
                           statuss=="Ligzda ar olām (LO)"~"LO",
                           statuss=="Ligzdā dzīvo 2 stārķi ilgāk par 1 mēnesi, bet mazuļu nav"~"AL",
                           statuss=="Ligzdā dzīvo 2 stārķi ilgāk par 1 mēnesi, nav ziņu par mazuļiem"~"AL",
                           statuss=="Ligzdā uzturas 1 stārķis"~"AL",
                           statuss=="Ligzdā uzturas 2 stārķi (ilgums nav zināms)"~"AL",
                           statuss=="Ligzdā uzturas 2 stārķi mazāk par 1 mēnesi"~"AL",
                           statuss=="Pāris ligzdošanai piemērotā biotopā (P)"~"P",
                           statuss=="PāRojas (ligzdotāji) (R)"~"R",
                           statuss=="Perēšanas laukums pieaugušam putnam (L)"~"L",
                           statuss=="Teritoriāla uzvedība (ligzdotājam) (T)"~"T",
                           statuss=="Tikko šķīlušies mazuļi vai vecāki ar mazuļiem (RM)"~"RM",
                           statuss=="Uztraukuma uzvedība vai saucieni (U)"~"U",
                           statuss=="Uzturas ligzdošanai piemērotā biotopā (B)"~"B"))
table(ddati$pazimei,useNA="always")
ddati2=ddati %>% 
  mutate(datums_no=as.Date(laiks_no),
         gads=lubridate::year(datums_no),
         datums_lidz=as.Date(laiks_lidz),
         nov_periods=ifelse(is.na(datums_lidz),0,datums_lidz-datums_no),
         pazime=pazimei,
         BreedCat=ifelse(pazime %in% c("LM","LO","JB","AL","LL","RM","AU"),"Pierādīta ligzdošana",
                         ifelse(pazime %in% c("G","L","U","V","R","P","T"),"Ticama ligzdošana",
                                ifelse(pazime=="B"|pazime=="D","Iespējama ligzdošana",NA))),
  ...
  filter(!is.na(tikls1000)) %>% 
  filter(!is.na(BreedCat))
arrow::write_parquet(ddati2,
                     "./IevadesDati/Noverojumi/parketi/harm_ddati.parquet")
rm(ddati)
rm(ddati2)
rm(dabasdati)

# ozols
ozols=read_parquet("./IevadesDati/Noverojumi/nosaukumi/vardnica_putni_ozols_vardnica.parquet")
table(ozols$SPOT_TYPE,useNA="always")
ozols=ozols %>% 
  mutate(BreedCat=case_when(SPOT_TYPE==51~"Iespējama ligzdošana",
                            SPOT_TYPE==52~"Ticama ligzdošana",
                            SPOT_TYPE==53~"Pierādīta ligzdošana"))
table(ozols$BreedCat,useNA="always")
ozolsA=ozols %>% 
  filter(!str_detect(DESCRIPTION,"(X, Y)")) %>% 
  filter(!str_detect(DESCRIPTION,"(X,Y)")) %>% 
  filter(!str_detect(DESCRIPTION,"Neapdz")) %>% 
  filter(!str_detect(DESCRIPTION,"neapdz"))
ozols2=ozolsA %>% 
  mutate(datums_no=as.Date(OBS_DATE),
         gads=lubridate::year(datums_no),
         datums_lidz=NA,
         nov_periods=ifelse(is.na(datums_lidz),0,datums_lidz-datums_no),
         pazime=NA,
  ...
  filter(!is.na(tikls1000)) %>% 
  filter(!is.na(BreedCat))
arrow::write_parquet(ozols2,
                     "./IevadesDati/Noverojumi/parketi/harm_ozols.parquet")
rm(ozols)
rm(ozolsA)
rm(ozols2)

# ACCGEN_testa kopa 1
lvm_testNov=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_test_ACCGEN_nov2024.xlsx")
table(lvm_testNov$pazime,useNA = "always")
lvm_testNov2=lvm_testNov %>% 
  mutate(datums_no=as.Date(DateTime),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazime,
  ...
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
lvm_testNov2_sf=st_as_sf(lvm_testNov2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
lvm_testNov2_sf$rinda=rownames(lvm_testNov2_sf)
lvm_testNov2_kvad=st_join(lvm_testNov2_sf,rezgis)
lvm_testNov_fin=data.frame(lvm_testNov2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(lvm_testNov_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_testNov.parquet")
rm(lvm_testNov)
rm(lvm_testNov2)
rm(lvm_testNov2_kvad)
rm(lvm_testNov2_sf)
rm(lvm_testNov_fin)

# ACCGEN_testa kopa 2
lvm_testLig=read_excel("./IevadesDati/Noverojumi/nosaukumi/vardnica_test_ACCGEN_ligzdas2024.xlsx")
table(lvm_testLig$pazime,useNA = "always")
lvm_testLig2=lvm_testLig %>% 
  mutate(datums_no=as.Date(datums),
         gads=lubridate::year(datums_no),
         datums_lidz=NA_real_,
         nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)),
         pazime=pazime,
  ...
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots) %>% 
  filter(!is.na(BreedCat))
lvm_testLig2_sf=st_as_sf(lvm_testLig2,coords=c("lksX","lksY"),remove = FALSE,crs=3059)
lvm_testLig2_sf$rinda=rownames(lvm_testLig2_sf)
lvm_testLig2_kvad=st_join(lvm_testLig2_sf,rezgis)
lvm_testLig_fin=data.frame(lvm_testLig2_kvad) %>% 
  filter(!duplicated(rinda)) %>% 
  dplyr::select(Grupa,NR,Name_key,KODS,zinatniski,gads,datums_no,datums_lidz,
                nov_periods,pazime,BreedCat,BreedCode,lksX,lksY,DatuKopa,avots,
                tikls100,tikls1000) %>% 
  filter(!is.na(tikls100)) %>% 
  filter(!is.na(tikls1000))
arrow::write_parquet(lvm_testLig_fin,
                     "./IevadesDati/Noverojumi/parketi/harm_testLig.parquet")
rm(lvm_testLig)
rm(lvm_testLig2)
rm(lvm_testLig2_kvad)
rm(lvm_testLig2_sf)
rm(lvm_testLig_fin)

# Treniņdatu apvienošana
eksp1=read_parquet("./IevadesDati/Noverojumi/parketi/harm_LVM.parquet")
eksp2=read_parquet("./IevadesDati/Noverojumi/parketi/harm_LVM_LUligzdas.parquet")
eksp3=read_parquet("./IevadesDati/Noverojumi/parketi/harm_LVM_LUnov.parquet")
LU_LVM=rbind(eksp2,eksp3,eksp1)
rm(eksp1)
rm(eksp2)
rm(eksp3)

LDF=read_parquet("./IevadesDati/Noverojumi/parketi/harm_DabasFonds.parquet")
kuze=read_parquet("./IevadesDati/Noverojumi/parketi/harm_kuze.parquet")
kalvans=read_parquet("./IevadesDati/Noverojumi/parketi/harm_kalvans.parquet")
ELe=read_parquet("./IevadesDati/Noverojumi/parketi/harm_LedinsJakovlevs.parquet")
zalvarnas=read_parquet("./IevadesDati/Noverojumi/parketi/harm_Racinskis.parquet")
dd=read_parquet("./IevadesDati/Noverojumi/parketi/harm_dd.parquet")
pucuplans=read_parquet("./IevadesDati/Noverojumi/parketi/harm_PucuPlans.parquet")

eksperti=rbind(LU_LVM,LDF,kuze,kalvans,ELe,zalvarnas,dd,pucuplans)
rm(LU_LVM)
rm(LDF)
rm(kuze)
rm(kalvans)
...
lpu=read_parquet("./IevadesDati/Noverojumi/parketi/harm_LPU.parquet")
plesi=read_parquet("./IevadesDati/Noverojumi/parketi/harm_Plesputni.parquet")
monitoringi=rbind(plesi,lpu)
rm(lpu)
rm(plesi)


dabasdati=read_parquet("./IevadesDati/Noverojumi/parketi/harm_ddati.parquet")
ozols=read_parquet("./IevadesDati/Noverojumi/parketi/harm_ozols.parquet")


apvienots1=rbind(eksperti,monitoringi)
apvienots1$datums_lidz=as.Date(apvienots1$datums_lidz)
apvienots2=rbind(apvienots1,dabasdati)
apvienots3=rbind(apvienots2,ozols)
apvienots=apvienots3 %>% 
  mutate(nov_periods=ifelse(is.na(datums_lidz),0,as.numeric(datums_lidz-datums_no)))
rm(dabasdati)
rm(eksperti)
rm(monitoringi)
rm(apvienots1)
...
apvienots$novID=paste0(apvienots$DatuKopa,"_",rownames(apvienots))
apvienots=apvienots %>% 
  filter(!is.na(datums_no))
arrow::write_parquet(apvienots,
                     "./IevadesDati/Noverojumi/apkopoti/apvienoti_putni.parquet")
rm(apvienots)



# ACCGEN testa kopas apvienošana
testam1=read_parquet("./IevadesDati/Noverojumi/parketi/harm_testNov.parquet")
testam2=read_parquet("./IevadesDati/Noverojumi/parketi/harm_testLig.parquet")
TestaKopa=rbind(testam1,testam2) %>%
  filter(KODS=="ACCGEN") %>% 
  arrange(BreedCode) %>% 
  filter(!duplicated(tikls1000))
arrow::write_parquet(TestaKopa,
                     "./IevadesDati/Noverojumi/parketi/TestaKopa_1km_2024.parquet")
```
